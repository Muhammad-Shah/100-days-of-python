...
def play_again():
    ...
    while not is_game_over:
        ...
        # If the game is not ended ask the user if they want to draw another card. If yes, then use the deal_card() function to add another card to the user_cards List. If no, then the game is ended
            ...
            should_go_on = input("Do you want to draw another card Type 'yes'or 'no': ")
            if should_go_on == "yes":
                user_cards.append(deal_card())
            else:
                is_game_over = True
    ...
    compare(user_scr=user_score, computer_scr=computer_score)

# Hint 14: Ask the user if they want to restart the game. If they answer yes, clear the console and start a new game of blackjack and show the logo from art.py
while input("Do tou want to play again? Type 'y' or 'n': ") == "y":
    clear()
    play_again()
